...
export default function UsagersRI2SList() {
  ...
  const filteredUsagers = usagers.filter((u) => {
    ...
    const matchesSearch =
      searchQuery === "" ||
      (u.pseudo?.pseudoName &&
        u.pseudo.pseudoName.toLowerCase().includes(searchLower)) ||
      (u.pseudo?.dossierNumber &&
        u.pseudo.dossierNumber.toLowerCase().includes(searchLower)) ||
    ...
    const matchesExperimentation =
      ...
      (u.experimentations &&
        u.experimentations.some((exp) => exp.code === experimentationFilter));
  ...
  useEffect(() => {
    const totalPagesCount = Math.max(
      1,
      Math.ceil(filteredUsagers.length / itemsPerPage)
    );
  ...
  const getStatusClassName = (status: string) => {
    switch (status) {
      ...
      case "Sorti":
        return "bl-status-exited";
      case "Suspendu":
        return "bl-status-suspended";
      default:
        return "bl-status-default";
    }
  };
        ...
        <div className="bl-controls">
          ...
          <div className="bl-filters">
            <div className="bl-filter-group">
              <label htmlFor="bl-role-filter" className="bl-filter-label">
                Rôle:
              </label>
              <select
                id="bl-role-filter"
            ...
            {experimentations.length > 0 && (
              <div className="bl-filter-group">
                <label htmlFor="bl-exp-filter" className="bl-filter-label">
                  Expérimentation:
                </label>
                <select
                  id="bl-exp-filter"
                ...
                >
                  ...
                  {experimentations.map((exp) => (
                    <option key={exp._id} value={exp.code}>
                      {exp.name}
                    </option>
                  ))}
                </select>
              ...
              <div className="bl-summary">
                <span className="bl-results-count">
                  ...
                  {filteredUsagers.length > itemsPerPage &&
                    ` - Affichage ${indexOfFirstItem + 1}-${Math.min(
                      indexOfLastItem,
                      filteredUsagers.length
                    )} sur ${filteredUsagers.length}`}
                </span>
                  ...
                  <tbody>
                    {currentItems.map((u) => (
                      <tr key={u._id}>
                        ...
                        <td>
                          {u.createdAt ? formatDateToYMD(u.createdAt) : "N/A"}
                        </td>
                        <td>
                          <div className="bl-actions">
                            <button
                              onClick={() =>
                                router.push(`/beneficiary/${u._id}`)
                              }
                            ...
                            <button
                              onClick={() =>
                                router.push(`/beneficiaire/${u._id}/edit`)
                              }
                            ...
                            <button
                              onClick={() =>
                                router.push(`/beneficiaires/associer/${u._id}`)
                              }
                    ...
                    <div className="bl-pagination-pages">
                      {getPageNumbers().map((number) => (
                        <button
                          ...
                          className={`bl-pagination-button ${
                            currentPage === number ? "bl-pagination-active" : ""
                          }`}
